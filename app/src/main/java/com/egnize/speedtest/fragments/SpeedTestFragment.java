package com.egnize.speedtest.fragments;
...
import com.egnize.speedtest.R;
import com.egnize.speedtest.custom_ui.TickProgressBar;
import com.egnize.speedtest.models.AdsSpeedTest;
import com.egnize.speedtest.utils.ConnectionDetector;
import com.egnize.speedtest.utils.GetSpeedTestHostsHandler;
import com.egnize.speedtest.utils.test.HttpDownloadTest;
import com.egnize.speedtest.utils.test.HttpUploadTest;
import com.egnize.speedtest.utils.test.PingTest;
...
import static com.egnize.speedtest.utils.LogUtils.LOGD;
import static com.egnize.speedtest.utils.LogUtils.LOGE;
import static com.egnize.speedtest.utils.LogUtils.LOGI;
import static com.egnize.speedtest.utils.LogUtils.makeLogTag;
		...
		new Thread (() -> {
			...
			if (info != null) {
				if (info.size () > 0) {
					...
					while (true) {
						if (!pingTestStarted) {
							LOGD ("@@", "pingTestStarted");
							pingTest.start();
							pingTestStarted = true;
						}
						if (pingTestFinished && !downloadTestStarted) {
							LOGD ("@@", "downloadTestStarted");
							downloadTest.start();
							downloadTestStarted = true;
						}
						if (downloadTestFinished && !uploadTestStarted) {
							LOGD ("@@", "uploadTestStarted");
							uploadTest.start();
							uploadTestStarted = true;
						}

						if (pingTestFinished) {
							LOGD ("@@", "pingTestFinished if");
						...
						if (pingTestFinished) {
							LOGD ("@@", "pingTestFinished 507");
							if (downloadTestFinished) {
								LOGD ("@@", "downloadTestFinished 509");
